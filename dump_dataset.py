...
def tf_serialize_example(sample):
    tf_string = tf.compat.v1.py_func(make_example, (sample["inputs"], sample["sat"], sample["policy"]), tf.string)
...
def main():
    ...
    with cnf_dataset.PoolDatasetGenerator(options) as generator, \
            tf.io.TFRecordWriter(os.path.join(dirname,filename)) as writer:
