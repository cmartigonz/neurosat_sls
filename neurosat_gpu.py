...
tf.flags.DEFINE_bool("use_gpu", True, "Use GPU if available rather than plain CPUs")
tf.flags.DEFINE_integer("iterations", 100, "Number of iterations per training loop.")
...
tf.flags.DEFINE_bool("enable_host_call", False, "Enable Estimator host_call.")
...
tf.flags.DEFINE_bool("test_files_gzipped", False, "Are test files gzipped.")
...
class Graph:
    def __init__(self, settings, features=None, labels=None):
        BATCH_SIZE = None
        if features is None:
            self.inputs = tf.compat.v1.placeholder(tf.float32, shape=(BATCH_SIZE, None, None, 2), name='inputs')
        else:
        ...
        if labels is None:
            self.policy_labels = tf.compat.v1.placeholder(tf.float32, shape=(BATCH_SIZE, None, 2), name='policy_labels')
            self.sat_labels = tf.compat.v1.placeholder(tf.float32, shape=(BATCH_SIZE,), name='sat_labels')
        else:
            self.sat_labels = labels["sat"]
            self.policy_labels = labels["policy"]

        batch_size = tf.shape(input=self.inputs)[0]
        variable_num = tf.shape(input=self.inputs)[1]
        clause_num = tf.shape(input=self.inputs)[2]
        assert_shape(variable_num, [])
        assert_shape(clause_num, [])

        variables_per_clause = tf.reduce_sum(input_tensor=self.inputs, axis=[1, 3])
        clauses_per_variable = tf.reduce_sum(input_tensor=self.inputs, axis=[2, 3])
        ...
        def basic_MLP(source, name, target_dimension=None, hidden_layers=None, end_activation=None):
            ...
            for index, size in enumerate(hidden_layers):
                last_hidden = tf.compat.v1.layers.dense(
                    last_hidden, size,
                    activation=HIDDEN_ACTIVATION,
                    name='{}_hidden_{}'.format(name, index),
                    reuse=tf.compat.v1.AUTO_REUSE)
            return tf.compat.v1.layers.dense(last_hidden, target_dimension, activation=end_activation, name='{}'.format(name),
                                   reuse=tf.compat.v1.AUTO_REUSE)
        ...
        def aggregate(Q, K, V, conn):
            if ATTENTION:
                ...
                unnorm_weights = conn
                norm_weights = tf.compat.v1.div(unnorm_weights,
                                      1.0 + tf.reduce_sum(input_tensor=unnorm_weights, axis=-1, keepdims=True))
            aggr_V = tf.matmul(norm_weights, V)
            return aggr_V

        with tf.compat.v1.variable_scope("graph_net", reuse=tf.compat.v1.AUTO_REUSE):
            initial_var_embedding = tf.compat.v1.get_variable(
            ...
            initial_clause_embedding = tf.compat.v1.get_variable(
        ...
        for level in range(LEVEL_NUMBER+1):
            if level >= 1:
                ...
                lit4cls_aggr_V = aggregate(cls4lit_Q, lit4cls_K, lit4cls_V,
                                           tf.transpose(a=all_connections, perm=[0, 2, 1]))
            ...
            self.positive_policy_logits = tf.compat.v1.layers.dense(
                positive_literal_embeddings, 1, name='policy', reuse=reuse)
            self.negative_policy_logits = tf.compat.v1.layers.dense(
            ...
            self.sat_logits = (tf.reduce_sum(
                input_tensor=tf.compat.v1.layers.dense(positive_literal_embeddings, 1, name='sat', reuse=reuse),
                axis=[1, 2]) + tf.reduce_sum(
                input_tensor=tf.compat.v1.layers.dense(negative_literal_embeddings, 1, name='sat', reuse=reuse),
                axis=[1, 2]))
            ...
            self.policy_loss = tf.compat.v1.losses.sigmoid_cross_entropy(
            ...
            self.sat_loss = tf.compat.v1.losses.sigmoid_cross_entropy(self.sat_labels, self.sat_logits)
            ...
            self.policy_error = tf.reduce_sum(input_tensor=tf.abs(
                tf.round(self.policy_probabilities_for_cmp) - self.policy_labels)) / (
                  tf.reduce_sum(input_tensor=self.sat_labels)) / (tf.cast(variable_num, dtype=tf.float32) * 2.0)
            self.sat_error = tf.reduce_mean(input_tensor=tf.abs(
            ...
            if FLAGS.add_summaries:
                ...
                tf.compat.v1.summary.scalar("loss"+lvln, self.level_loss)
                tf.compat.v1.summary.scalar("policy_loss"+lvln, self.policy_loss)
                tf.compat.v1.summary.scalar("policy_error"+lvln, self.policy_error)
                # tf.summary.scalar("policy_top1_error"+lvln, self.policy_top1_error)
                tf.compat.v1.summary.scalar("sat_loss"+lvln, self.sat_loss)
                tf.compat.v1.summary.scalar("sat_error"+lvln, self.sat_error)
                tf.compat.v1.summary.scalar("sat_fraction"+lvln, tf.reduce_sum(input_tensor=self.sat_labels) / tf.cast(batch_size, dtype=tf.float32))

        if FLAGS.add_summaries:
            # tf.summary.scalar("loss", self.loss)
            tf.compat.v1.summary.scalar("policy_loss", self.policy_loss)
            tf.compat.v1.summary.scalar("policy_error", self.policy_error)
            # tf.summary.scalar("policy_top1_error", self.policy_top1_error)
            tf.compat.v1.summary.scalar("sat_loss", self.sat_loss)
            tf.compat.v1.summary.scalar("sat_error", self.sat_error)
            tf.compat.v1.summary.scalar("sat_fraction",
                              tf.reduce_sum(input_tensor=self.sat_labels) / tf.cast(batch_size, dtype=tf.float32))
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
        def metric_fn(sat_labels, policy_labels, policy_weights, *args):
            ...
            for num, (sat, policy) in enumerate(zip(args[0::2], args[1::2])):
                leveln = str(num)
                metrics.update({
                    'sat_error_' + leveln: tf.compat.v1.metrics.mean_absolute_error(
                        labels=sat_labels,
                        predictions=sat),
                    'policy_error_' + leveln: tf.compat.v1.metrics.mean_absolute_error(
        ...
        return tf.estimator.EstimatorSpec(
            mode, loss=graph.loss, eval_metric_ops=metric_fn(graph.sat_labels, graph.policy_labels, graph.policy_weights, *graph.sat_list, *graph.policy_list),
...
def serving_input_receiver_fn():
    feature = tf.compat.v1.placeholder(tf.float32, shape=[None, None, None, 2])
